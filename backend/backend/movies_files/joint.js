...
(function(root, factory) {
  ...
  _.extend(Collection.prototype, Events, {
    ...
    remove: function(models, options) {
      ...
      // ######################################################################################## EDITED SECTION (@18/11/2014)
      // THIS SECTION WAS ADDED TO REMOVE THE LINK FROM LINKS LIST
      remove_link();
...
joint.dia.ElementView = joint.dia.CellView.extend({
    ...
    resize: function() {
        ...
        var scalable2 = V(this.$('.scalable')[1]);										// CUSTOM: TO ALLOW MULTIPLE SCALABLE TAGS
        if ((!scalable) && (!scalable2)) {
            // If there is no scalable elements, then there is nothing to resize.re
            return;
        }
        var scalableBbox = scalable.bbox(true);
        if(scalable2) var scalableBbox2 = scalable2.bbox(true);										// CUSTOM: TO ALLOW MULTIPLE SCALABLE TAGS
        ...
        if(scalable2) scalable2.attr('transform', 'scale(' + (size.width / (scalableBbox2.width || 1)) + ',' + (size.height / (scalableBbox2.height || 1)) + ')');										// CUSTOM: TO ALLOW MULTIPLE SCALABLE TAGS
...
joint.dia.Link = joint.dia.Cell.extend({
    ...
    markup: [
        '<path class="connection" stroke="black" fill="none" />',
        '<path class="marker-source" fill="black" stroke="black" />',
        '<path class="marker-target" fill="black" stroke="black" />',
        '<path class="connection-wrap" fill="none" />',
        '<g class="labels"/>',
        '<g class="marker-vertices" fill="none" />',
        // ######################################################################################## EDITED SECTION
        // THIS SECTION WAS COMMENTED TO REMOVE THE ARROWHEADS THAT ALLOW TO CHANGE LINK TARGETS (@18/11/2014)
    ...
    labelMarkup: [
        '<g class="label" >',
        '<rect x="0" y="0" width="100" height="30" fill="#cccccc"/>',
        '<text />',
        '</g>'
    ].join(''),
    ...
    vertexMarkup: [
        ...
        '<circle opacity="0" class="marker-vertex" idx="<%= idx %>" r="10" />',
        '<path opacity="0" class="marker-vertex-remove-area" idx="<%= idx %>" d="M16,5.333c-7.732,0-14,4.701-14,10.5c0,1.982,0.741,3.833,2.016,5.414L2,25.667l5.613-1.441c2.339,1.317,5.237,2.107,8.387,2.107c7.732,0,14-4.701,14-10.5C30,10.034,23.732,5.333,16,5.333z" transform="translate(5, -33)"/>',
        '<path opacity="0" class="marker-vertex-remove" idx="<%= idx %>" transform="scale(.8) translate(9.5, -37)" d="M24.778,21.419 19.276,15.917 24.777,10.415 21.949,7.585 16.447,13.087 10.945,7.585 8.117,10.415 13.618,15.917 8.116,21.419 10.946,24.248 16.447,18.746 21.948,24.248z">',
...
joint.dia.LinkView = joint.dia.CellView.extend({
	...
	function can(feature) {
        ...
        //console.log(this);
        show_link_properties(null,this.model.id);
        //this.paper.trigger('link:options', evt, this, x, y);
    },
...
// // ##########################################################################
// // CUSTOM SHAPE - USED FOR AN SQL TABLE BLOCK

// joint.shapes.basic.TableElement = joint.shapes.basic.Generic.extend({

//     markup: '<g class="rotatable"><g class="scalable"><rect/><line/></g> <text  id="title" /> <text  id="fields" /> </g>',

//     defaults: joint.util.deepSupplement({
//         type: 'basic.TableElement',
//         attrs: {
//             'rect': { fill: '#FFFFFF', stroke: 'black', 'stroke-width': 2, width: 100, height: 100 },
//             'line': { stroke: '#000000', 'stroke-width': 2, x1:0, x2:100, y1:50, y2:50 },
//             '#title' : { 'font-size': 20, /*text: '',*/ 'ref-x': 0.5, /*'ref-y': .25,*/ ref: 'rect', 'y-alignment': 'top', 'x-alignment': 'middle', 'fill': 'black', 'font-family': 'Arial, helvetica, sans-serif' },
//             '#fields': { 'font-size': 20, /*text: '',*/ 'ref-x': .05, /*'ref-y': .25,*/ ref: 'rect', 'y-alignment': 'top', 'x-alignment': 'left', 'fill': 'black', 'font-family': 'Arial, helvetica, sans-serif' }
//             //,'text': { 'font-size': 14, text: '', 'ref-x': 0.0, 'ref-y': .25, ref: 'rect', 'y-alignment': 'top', 'x-alignment': 'left', fill: 'black', 'font-family': 'Arial, helvetica, sans-serif' }
//             // 'ref-y': .125   : 1/(Nro de seccoes -> 1(Header) + 3) / 2 (pq e' o centro do texto)
//         }
