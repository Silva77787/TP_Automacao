...
joint.dia.CommandManager = Backbone.Model.extend({
    ...
    addCommand: function(cmdName, cell, graph, options) {
        // allow undo-redo operations only in the conceptual diagram
        ...
        if (cmdName.substr(0, this.PREFIX_LENGTH) == 'change:') {
            if (cmdName.substr(this.PREFIX_LENGTH) == 'attrs') return
        }
    ...
    undo: function() {
        ...
        if (command) {
            /* undo creation of table equals removing the table
             if ( isTable(command.data.id) && command.action == "add")
             delete_table(command.data.id);

             // same for links
             else if ( isLink(command.data.id) && command.action == "add" )
             remove_link(command.data.id);*/
